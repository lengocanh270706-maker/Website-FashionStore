...
$result = $conn->query("SELECT categories.*, (SELECT COUNT(*) FROM products WHERE products.category_id = categories.id) as product_count FROM categories ORDER BY id DESC");
?>
<!DOCTYPE html>
<html lang="vi">
<head>
    <meta charset="UTF-8">
    <title>Quản lý Danh mục - Mây Admin</title>
        ...
        <div class="row">
            ...
            <div class="col-md-2 sidebar">
                ...
                <nav>
                    ...
                    <a href="index.php" class="active"><i class="bi bi-tags"></i> Quản lý danh mục</a>
                    ...
                    <table class="table align-middle mb-0">
                        ...
                        <tbody>
                            <?php if ($result && $result->num_rows > 0): ?>
                                ...
                                <tr>
                                    <td class="text-muted">#DM<?= $row['id'] ?></td>
                                    <td class="fw-medium"><?= htmlspecialchars($row['name']) ?></td>
                                    <td class="text-muted" style="max-width: 300px;"><?= htmlspecialchars($row['description'] ?? 'Chưa có mô tả') ?></td>
                                    <td><span class="badge bg-secondary"><?= $row['product_count'] ?> sản phẩm</span></td>
                                    <td>
                                        <?= ($row['status'] == 1) ? '<span class="badge bg-success bg-opacity-25 text-success">Hiển thị</span>' : '<span class="badge bg-secondary bg-opacity-25 text-secondary">Ẩn</span>' ?>
                                    </td>
                                    <td>
                                        <a href="edit.php?id=<?= $row['id'] ?>" class="text-primary fs-5 me-2" title="Sửa"><i class="bi bi-pencil-square"></i></a>
                                        <a href="delete.php?id=<?= $row['id'] ?>" class="text-danger fs-5" title="Xóa" onclick="return confirm('Bạn có chắc chắn muốn xóa danh mục này?');"><i class="bi bi-trash"></i></a>
                                    </td>
                                </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr><td colspan="6" class="text-center py-4 text-muted">Chưa có danh mục nào trong hệ thống.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
